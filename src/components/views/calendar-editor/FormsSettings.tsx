           ...
           <div className="space-y-6">
              {activeGroup.fields.map(field => (
                <div key={field.id} className="relative group srf-panel border hairline p-5 rounded-2xl shadow-sm hover:border-slate-300 transition-colors">
                  ...
                  <input type="text" readOnly value={`Dato para: ${field.label}`} className={`w-full bg-[#f5f5f7] border border-transparent ink-3 px-4 py-3 text-[13px] rounded-xl outline-none transition-colors ${!field.isDefault ? 'cursor-not-allowed opacity-70' : 'cursor-default'}`} />
                </div>
              ))}
