    ...
    days.push(
      <div 
        ...
        className={`w-8 h-8 flex items-center justify-center rounded-full cursor-pointer text-[13px] font-medium transition-colors ${
          ...
          isToday ? 'bg-slate-100 text-black' : 'text-slate-600 hover:bg-slate-100'
        }`}
      >
        {i}
      </div>
  ...
  return (
    <div className="relative w-full" ref={dropdownRef}>
      <div 
        ...
        className={className || "flex items-center bg-white border border-slate-200 hover:border-slate-300 rounded-xl px-4 py-3 text-sm w-full transition-all shadow-sm cursor-pointer"}
      >
         <CalendarIcon className="w-4 h-4 text-slate-500 mr-2.5" />
      ...
      {isOpen && (
        <div className="absolute top-full left-0 mt-2 bg-white border border-slate-200 rounded-xl shadow-xl z-[9999] p-4 w-[280px]">
           {value && (
             <div className="bg-[#eef2f6] text-slate-700 font-semibold text-xs py-2 px-3 rounded-lg text-center mb-4">
           ...
           <div className="flex justify-between items-center mb-4 px-1">
             ...
             <div className="flex gap-1.5">
               <button onClick={handlePrevMonth} className="p-1 rounded-full bg-black hover:bg-slate-900 text-white transition-colors cursor-pointer"><ChevronLeft className="w-3.5 h-3.5" /></button>
               <button onClick={handleNextMonth} className="p-1 rounded-full bg-black hover:bg-slate-900 text-white transition-colors cursor-pointer"><ChevronRight className="w-3.5 h-3.5" /></button>
             </div>
           </div>
           
           <div className="grid grid-cols-7 gap-1 text-center mb-2">
             {dayNames.map(d => (
               <div key={d} className="text-[11px] font-semibold text-slate-500">{d}</div>
             ))}
           </div>
...
const CustomTimePicker = ({ value, onChange, disabled = false }: { value: string, onChange?: (val: string) => void, disabled?: boolean }) => {
  ...
  if (disabled) {
    return (
      <div className="relative w-full flex-1 max-w-[150px] opacity-45 pointer-events-none select-none">
        <div className="w-full bg-slate-50 border border-slate-200/80 text-slate-500 text-[13px] rounded-xl py-2 pl-9 pr-3 font-semibold text-center shadow-sm min-h-[38px] flex items-center justify-center">
  ...
  return (
    <div className="relative w-full flex-1 max-w-[150px]" ref={dropdownRef}>
      <div 
        onClick={(e) => { e.preventDefault(); e.stopPropagation(); setIsOpen(!isOpen); }}
        className="w-full bg-[#f5f5f7] border border-transparent hover:border-slate-300 focus-within:border-slate-300 focus-within:bg-white text-slate-800 text-[13px] rounded-xl py-2 pl-9 pr-3 outline-none font-semibold text-center transition-all duration-200 shadow-sm cursor-pointer hover:shadow-md select-none relative z-10 flex items-center justify-center min-h-[38px]"
      >
        <Clock className="w-4 h-4 text-slate-400 absolute left-3 top-[11px] z-20 pointer-events-none" />
        <span className="font-semibold text-slate-800">{value || '8:00 a. m.'}</span>
      </div>

      {isOpen && (
        <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 bg-white border border-slate-200/80 rounded-2xl shadow-xl z-[99999] p-4 w-[200px] flex flex-col items-center">
          {/* Picker Columns */}
          <div className="flex items-center justify-center gap-2 mb-3">
            ...
            <div className="flex flex-col items-center">
              ...
              <span className="font-bold text-slate-800 text-[15px] my-1 w-7 text-center select-none">
            ...
            {/* Separator */}
            <span className="font-bold text-slate-400 text-base mb-1 select-none">:</span>
            ...
            <div className="flex flex-col items-center">
              ...
              <span className="font-bold text-slate-800 text-[14px] my-1 w-8 text-center select-none">
                {ampm}
              </span>
              <button 
                type="button"
          ...
          <button 
            ...
            className="w-full text-center py-1.5 text-xs font-bold text-slate-600 hover:text-black bg-slate-50 hover:bg-slate-200/60 rounded-xl transition-all border border-slate-200/60 hover:border-slate-300 cursor-pointer"
          >
            CERRAR
          </button>
        </div>
...
const SchedulingSettings: React.FC<Props> = ({ initialData, onSave, calendarGroups, onGroupsChange }) => {
  ...
  return (
    <div className="bg-white pb-10 rounded-b-2xl">
      ...
      <div className="sticky top-0 z-20 bg-[#f5f5f7]/95 backdrop-blur-md border-b border-slate-200/60 shadow-[0_1px_0_0_rgba(0,0,0,0.04)]">
        ...
        <div className="flex items-end overflow-x-auto no-scrollbar pt-3 px-4 gap-1">
          <button
            ...
            className="flex items-center shrink-0 text-white bg-slate-900 hover:bg-black px-3.5 py-2 text-[12px] font-bold rounded-t-xl transition-colors cursor-pointer gap-1.5 shadow-sm"
          >
          ...
          {groups.map(g => (
            <button
              ...
              className={`px-5 py-2 text-[13px] font-semibold border-t border-x rounded-t-xl -mb-px transition-all duration-150 cursor-pointer whitespace-nowrap shrink-0 ${
                activeGroupId === g.id
                  ? 'bg-white border-slate-200 text-black border-t-[2.5px] border-t-black shadow-[0_-2px_6px_rgba(0,0,0,0.02)]'
                  : 'bg-slate-100/60 border-transparent text-slate-500 hover:bg-slate-100 hover:text-slate-800'
              }`}
            >
              {g.title || g.name}
            </button>
        ...
        <div className="flex items-center justify-end gap-2 px-4 py-2.5 border-t border-slate-200/60 bg-white/60">
          <button
            onClick={handleCancel}
            className="px-4 py-2 text-slate-600 text-[13px] font-semibold rounded-xl transition-colors hover:bg-slate-100 cursor-pointer"
          >
            Cancelar
          </button>
          <button
            onClick={handleSave}
            className="px-5 py-2 bg-black hover:bg-slate-900 text-white rounded-xl text-[13px] font-bold flex items-center gap-2 transition-all shadow-md shadow-black/20 cursor-pointer"
          >
      ...
      <div className="p-8 space-y-10 bg-white max-w-5xl mx-auto">
        {errorMessage && (
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between text-sm font-semibold text-slate-500 mb-4 gap-4 bg-slate-50 p-4 rounded-2xl border border-slate-200/60">
           <div className="flex items-center gap-3">
             <span className="text-slate-400 text-[13px] uppercase tracking-wider">Editando Grupo:</span> 
             <input 
               type="text" 
               value={activeGroup.name}
               onChange={(e) => updateGroup({ name: e.target.value, title: e.target.value })}
               className="border-b border-transparent hover:border-slate-300 focus:border-black py-1.5 focus:outline-none transition-colors text-slate-800 font-bold bg-transparent text-base" 
             />
           </div>
           <div className="flex gap-4">
             ...
             <button onClick={() => handleDuplicateGroup(activeGroup.id)} className="flex items-center px-4 py-2 hover:bg-slate-100 text-black rounded-lg transition-colors cursor-pointer text-[13px]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
           <div className="space-y-7">
             <div>
               <label className="flex items-center text-[13px] font-semibold text-slate-700 mb-2.5">
                 Título <span className="text-red-500 ml-1 opacity-80">*</span> <Info className="w-3.5 h-3.5 text-slate-400 ml-1.5 cursor-help" />
               </label>
               <input 
                 type="text" 
                 ...
                 className="w-full bg-[#f5f5f7] border border-transparent focus:bg-white rounded-xl px-4 py-3 text-sm text-slate-900 focus:ring-2 focus:ring-black/20 focus:border-slate-300 transition-all duration-200 outline-none" 
               />
             </div>
             <div>
               <label className="flex items-center text-[13px] font-semibold text-slate-700 mb-2.5">
                 Administradores <span className="text-red-500 ml-1 opacity-80">*</span> <Info className="w-3.5 h-3.5 text-slate-400 ml-1.5 cursor-help" />
               </label>
               <div className="bg-[#f5f5f7] border border-slate-200/60 rounded-xl p-3 flex flex-wrap gap-2 text-sm min-h-[60px] items-start transition-colors hover:border-slate-300">
                  {activeGroup.administrators.map(admin => (
                    <div key={admin.id} className="bg-white rounded-lg py-1.5 px-2.5 flex items-center shadow-sm border border-slate-200">
                      {admin.avatar ? (
                        <img src={admin.avatar} alt="" className="w-6 h-6 rounded-full mr-2.5 object-cover"/>
                      ) : (
                        <div className="w-6 h-6 rounded-full bg-gradient-to-tr from-slate-100 to-slate-50 flex items-center justify-center mr-2.5 border border-slate-200 shadow-sm text-slate-500 font-bold text-[10px]">
                           {admin.name.charAt(0)}
                        </div>
                      )}
                      <span className="font-semibold text-slate-700 mr-2.5 text-[13px]">{admin.name}</span>
                      <button onClick={() => removeAdmin(admin.id)} className="text-slate-400 hover:text-rose-500 cursor-pointer transition-colors">
                  ...
                  <button 
                    ...
                    className="text-[13px] bg-white hover:bg-slate-50 border border-dashed border-slate-300 text-slate-500 px-4 py-2 flex items-center rounded-lg font-semibold cursor-pointer transition-colors shadow-sm"
                  >
           ...
           <div>
             <label className="flex items-center text-[13px] font-semibold text-slate-700 mb-2.5">
               Descripción <Info className="w-3.5 h-3.5 text-slate-400 ml-1.5 cursor-help" />
             </label>
             <textarea 
               rows={6} 
               ...
               className="w-full bg-[#f5f5f7] border border-transparent focus:bg-white rounded-xl px-4 py-3 text-sm text-slate-900 focus:ring-2 focus:ring-black/20 focus:border-slate-300 transition-all duration-200 outline-none resize-none" 
             />
           </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
           <div className="md:col-span-1">
               <div className="flex justify-between items-center mb-2.5 ml-1">
                  <label className="flex items-center text-[13px] font-semibold text-slate-700">
                     Punto de partida <Info className="w-3.5 h-3.5 text-slate-400 ml-1.5 cursor-help" />
                  </label>
                  <label className="flex items-center text-[11px] text-slate-500 hover:text-slate-700 cursor-pointer select-none">
                     <input 
                       type="checkbox" 
               ...
               { (activeGroup.customStartingPoint || !STANDARD_STARTING_POINTS.includes(activeGroup.startingPoint)) ? (
                 <div className="relative">
                   <input 
                     ...
                     className="w-full bg-[#f5f5f7] border border-transparent focus:bg-white hover:border-slate-300 focus:border-slate-300 rounded-xl pl-4 pr-16 py-3 text-sm text-slate-800 font-semibold outline-none transition-all duration-200 focus:ring-2 focus:ring-black/20 shadow-sm"
                   />
                   <span className="absolute right-3 top-3.5 text-xs text-slate-400 font-bold pointer-events-none">
                     minutos
                   </span>
                 </div>
               ) : (
                  <select 
                    ...
                    className="w-full bg-[#f5f5f7] border border-transparent focus:bg-white hover:border-slate-300 focus:border-slate-300 rounded-xl px-4 py-3 text-sm text-slate-800 font-semibold outline-none cursor-pointer transition-all duration-200 focus:ring-2 focus:ring-black/20 shadow-sm"
                  >
           ...
           <div className="md:col-span-2">
              <label className="flex items-center text-[13px] font-semibold text-slate-700 mb-2.5 ml-1">
                 Tiempo de la sesión <Info className="w-3.5 h-3.5 text-slate-400 ml-1.5 cursor-help" />
              </label>
              <div className="flex gap-3">
                 <select 
                   value={activeGroup.sessionTimeHours}
                   onChange={(e) => updateGroup({ sessionTimeHours: e.target.value })}
                   className="w-full bg-white border border-slate-200 hover:border-slate-300 focus:border-slate-300 rounded-xl px-4 py-3 text-sm text-slate-800 font-semibold outline-none cursor-pointer transition-all focus:ring-2 focus:ring-black/20 shadow-sm"
                 >
                    {[...Array(24)].map((_, i) => (
           ...
           <div className="md:col-span-2">
              <label className="flex items-center text-[13px] font-semibold text-slate-700 mb-2.5 ml-1">
                 Intervalo entre sesiones <Info className="w-3.5 h-3.5 text-slate-400 ml-1.5 cursor-help" />
              </label>
              <div className="flex gap-3">
                 <select 
        ...
        <div className="flex items-center pt-2">
          <label className="flex items-center cursor-pointer group bg-slate-50 border border-slate-200 rounded-xl px-5 py-4 w-full md:w-auto transition-colors hover:bg-slate-100">
            <div className="relative">
               <input 
                 type="checkbox" 
                 className="sr-only" 
               ...
               />
               <div className={`block w-12 h-7 rounded-full transition-colors ${preventCrossGroupOverlaps ? 'bg-black' : 'bg-slate-300'}`}></div>
               <div className={`dot absolute left-1 top-1 bg-white w-5 h-5 rounded-full transition-transform shadow-sm ${preventCrossGroupOverlaps ? 'translate-x-5' : 'translate-x-0'}`}></div>
            </div>
            <span className="ml-4 text-[13px] font-semibold text-slate-700 select-none">Bloquear horarios cruzados entre diferentes grupos del calendario.</span>
          </label>
           <Info className="w-4 h-4 text-slate-400 ml-3 cursor-help hidden md:block" />
        </div>
        ...
        <div>
         ...
         <div>
            <div className="flex items-center mb-4">
               <h3 className="text-[17px] font-bold text-slate-900 flex items-center tracking-tight">
                  Horarios Habilitados <span className="text-red-500 ml-1 opacity-80">*</span> <Info className="w-4 h-4 text-slate-400 ml-2" />
               </h3>
            </div>

            {/* Zona horaria — mismo estilo que Meses Disponibles */}
            <div className="relative mb-4">
               <Globe className="absolute left-4 top-1/2 -translate-y-1/2 w-4.5 h-4.5 text-slate-400 pointer-events-none" />
               <select value={timezone} onChange={(e) => setTimezone(e.target.value)} className="w-full appearance-none bg-white border border-slate-200 rounded-xl pl-11 pr-10 py-3 text-[14px] font-medium text-slate-700 outline-none focus:ring-2 focus:ring-black/20 shadow-sm cursor-pointer">
            ...
            <div className="bg-[#f5f5f7]/60 border border-slate-200/60 rounded-2xl overflow-hidden">
               {activeGroup.days.map((day, dayIndex) => (
                  <div
                    key={day.name}
                    className={`px-4 py-3 ${dayIndex < activeGroup.days.length - 1 ? 'border-b border-slate-200/60' : ''}`}
                  >
                        ...
                        <label className="flex items-center gap-2.5 cursor-pointer">
                           <div className="relative flex items-center justify-center">
                              <input
                                ...
                                className="peer appearance-none w-[20px] h-[20px] rounded-md border-2 border-slate-300 checked:bg-black checked:border-black transition-all cursor-pointer shadow-sm"
                              />
                           ...
                           <span className={`text-[14px] font-semibold select-none ${day.active ? 'text-slate-900' : 'text-slate-400'}`}>{day.name}</span>
                        </label>
                        {!day.active && (
                           <span className="text-[11px] font-bold text-slate-400 bg-white border border-slate-200 rounded-lg px-2.5 py-1">Día deshabilitado</span>
                        )}
                     </div>
                        ...
                        <div className="mt-2.5 space-y-2">
                           {day.times.map((time, ti) => (
                              <div key={ti} className="flex items-center gap-2">
                                 ...
                                 <span className="text-slate-400 text-[11px] font-bold shrink-0">a</span>
                                 <div className="flex-1 min-w-0">
                                    <CustomTimePicker value={time.end} onChange={(val) => updateDayTime(dayIndex, ti, { end: val })} />
                                 </div>
                                 {ti === 0 ? (
                                    <button onClick={() => addTimeRange(dayIndex)} className="shrink-0 text-slate-900 w-8 h-8 flex items-center justify-center rounded-xl bg-white border border-slate-200 active:bg-slate-100 transition-colors">
            ...
            <div className="bg-slate-50 border border-slate-200/80 rounded-2xl p-4 flex items-start gap-3 mt-4">
               <Lightbulb className="w-4.5 h-4.5 text-slate-500 shrink-0 mt-0.5" />
               <div>
                  <p className="font-bold text-slate-800 text-[13px]">Consejo</p>
                  <p className="text-[12px] text-slate-500 mt-0.5">Puedes agregar múltiples rangos de horario en un mismo día presionando el botón +.</p>
               </div>
            </div>
         </div>
           ) : (
           ...
           <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-8 gap-4">
              ...
              <span className="text-[13px] font-semibold text-slate-500 flex items-center bg-slate-100 px-3 py-1.5 rounded-lg border border-slate-200/60">
                      ...
                      <label className="flex items-center cursor-pointer group">
                        <div className="relative flex items-center justify-center">
                          <input 
                            ...
                            checked={day.active} 
                            onChange={(e) => updateDay(dayIndex, { active: e.target.checked })}
                            className="peer appearance-none w-[22px] h-[22px] rounded-md border-2 border-slate-300 checked:bg-black checked:border-black transition-all cursor-pointer shadow-sm" 
                          />
                        ...
                        <span className={`ml-3.5 text-[14px] font-semibold select-none transition-colors ${day.active ? 'text-slate-800' : 'text-slate-500 group-hover:text-slate-700'}`}>{day.name}</span>
                      </label>
                    </div>
                       ...
                       {day.active ? (
                          day.times.map((time, timeIndex) => (
                            <div key={timeIndex} className="flex items-center gap-3 relative group/time">
                               ...
                               />
                               <span className="text-slate-400 text-[13px] font-semibold mx-1">a</span>
                               <CustomTimePicker 
                                 value={time.end} 
                                 onChange={(val) => updateDayTime(dayIndex, timeIndex, { end: val })}
                               />
                               {timeIndex === 0 ? (
                                 <button onClick={() => addTimeRange(dayIndex)} className="text-black hover:text-slate-900 hover:bg-slate-100 w-9 h-9 flex items-center justify-center rounded-lg transition-colors cursor-pointer ml-1 relative z-10">
                          ...
                          day.times.map((time, timeIndex) => (
                            <div key={timeIndex} className={`flex items-center gap-3 relative opacity-40 pointer-events-none`}>
                               ...
                               <CustomTimePicker value={time.start} disabled={true} />
                                <span className="text-slate-400 text-sm font-semibold mx-1">a</span>
        ...
        <hr className="border-t border-slate-200/60 my-10" />

        {/* Fechas Disponible */}
        <div>
           <div className="mb-6">
              <h3 className="text-lg font-semibold text-slate-900 flex items-center tracking-tight">
                 Meses disponibles <span className="text-red-500 ml-1 opacity-80">*</span> <Info className="w-4 h-4 text-slate-400 ml-2" />
              </h3>
           </div>

           <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-x-6 gap-y-4 max-w-4xl px-7 py-7 bg-[#f5f5f7]/50 border border-slate-200/60 rounded-2xl">
                 ...
                 <div key={month.name} className="flex flex-col gap-2">
                    <label className="flex items-center cursor-pointer mb-1">
                      <div className="relative flex items-center justify-center">
                        <input 
                          ...
                          className="peer appearance-none w-[18px] h-[18px] rounded-[5px] border border-slate-300 checked:bg-black checked:border-transparent transition-all cursor-pointer shadow-sm" 
                        />
                      ...
                      <span className={`ml-3 text-[13px] font-semibold select-none ${month.active ? 'text-slate-800' : 'text-slate-400'}`}>{month.name}</span>
                    </label>
                    ...
                    <div className={`flex items-center gap-2 ${!month.active && 'opacity-40 pointer-events-none'}`}>
                       <select 
                         ...
                         className="w-full bg-white border border-slate-200 hover:border-slate-300 rounded-lg px-2.5 py-1.5 text-xs text-slate-700 font-semibold outline-none cursor-pointer transition-colors shadow-sm focus:ring-2 focus:ring-black/20"
                       >
                          {[...Array(31)].map((_, i) => (
                            <option key={i + 1} value={String(i + 1)}>{i + 1}</option>
                          ))}
                       </select>
                       <span className="text-slate-400 text-xs font-bold px-1">a</span>
                       <select 
                         value={month.end}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 max-w-4xl">
           <div>
              <label className="flex items-center text-xs font-semibold text-slate-600 mb-2.5 ml-1">
                 Cantidad de Sesiones <Info className="w-3.5 h-3.5 text-slate-400 ml-1 cursor-help" />
              </label>
              <input 
                type="number" 
                min="1"
                ...
                className="w-full max-w-[120px] bg-white border border-slate-200 focus:border-slate-300 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-black/20 text-slate-800 font-semibold shadow-sm transition-all text-center" 
              />
           </div>

           <div>
              <label className="flex items-center text-[13px] font-semibold text-slate-700 mb-2.5 ml-1">
                 Disponibilidad <Info className="w-3.5 h-3.5 text-slate-400 ml-1.5 cursor-help" />
              </label>
              ...
              {activeGroup.availabilityType === '... días corridos' ? (
                <div className="flex">
                   <input 
                     ...
                     className="w-24 bg-white border border-slate-200 border-r-0 rounded-l-xl px-4 py-3 text-sm focus:outline-none text-slate-800 font-semibold text-center z-10 focus:border-slate-300 focus:ring-2 focus:ring-black/20 shadow-sm"
                   />
                   <select 
                     value={activeGroup.availabilityType}
                     onChange={(e) => updateGroup({ availabilityType: e.target.value })}
                     className="w-full bg-white border border-slate-200 focus:border-slate-300 rounded-r-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-black/20 text-slate-800 font-semibold shadow-sm transition-all cursor-pointer"
                   >
                ...
                <div className="flex">
                   <select 
                     value={activeGroup.availabilityType}
                     onChange={(e) => updateGroup({ availabilityType: e.target.value })}
                     className="w-full bg-white border border-slate-200 border-r-0 focus:border-slate-300 rounded-l-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-black/20 text-slate-800 font-semibold shadow-sm transition-all cursor-pointer z-10"
                   >
                      <option value="Indefinidamente">Indefinidamente</option>
                      <option value="Hasta cierto día...">Hasta cierto día...</option>
                      <option value="... días corridos">... días corridos</option>
                   </select>
                   <div className="w-full relative bg-white rounded-r-xl shadow-sm">
                     <CustomDatePicker 
                       value={activeGroup.availabilityDate}
                       onChange={(val) => updateGroup({ availabilityDate: val })}
                       placeholder="Seleccione fecha"
                       className="flex items-center bg-transparent border border-slate-200 hover:border-slate-300 rounded-r-xl px-4 py-3 text-sm w-full transition-all cursor-pointer h-full border-l-0 focus-within:ring-2 focus-within:ring-black/20"
                     />
                   </div>
                </div>
              ) : (
                <select 
                  value={activeGroup.availabilityType}
                  onChange={(e) => updateGroup({ availabilityType: e.target.value })}
                  className="w-full bg-white border border-slate-200 focus:border-slate-300 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-black/20 text-slate-800 font-semibold shadow-sm transition-all cursor-pointer"
                >
                   <option value="Indefinidamente">Indefinidamente</option>
                   <option value="Hasta cierto día...">Hasta cierto día...</option>
                   <option value="... días corridos">... días corridos</option>
              ...
              {activeGroup.availabilityType === 'Hasta cierto día...' && activeGroup.availabilityDate && (
                <p className="text-[13px] text-slate-500 mt-2 font-medium leading-relaxed">Los horarios sólo pueden ser recibidos antes de las 11:59 pm del día {activeGroup.availabilityDate.split('-').reverse().join('/')}</p>
              )}
              {activeGroup.availabilityType === '... días corridos' && (
                <p className="text-[13px] text-slate-500 mt-2 font-medium leading-relaxed">Estás disponible para nuevas citas durante {activeGroup.availabilityRollingDays || 0} días calendario a partir de hoy</p>
              )}
           </div>
           
           <div className="md:col-span-2">
              <label className="flex items-center text-[13px] font-semibold text-slate-800 mb-3 block">
                 Anticipación mínima <Info className="w-3.5 h-3.5 text-slate-400 ml-1.5 cursor-help" />
              </label>
              <div className="flex max-w-sm">
                 ...
                 <select 
                   ...
                   className={`w-full bg-white border border-slate-200 focus:border-slate-300 px-4 py-3 text-sm text-slate-700 font-semibold outline-none cursor-pointer focus:ring-2 focus:ring-black/20 shadow-sm ${['Día(s) y', 'Horas', 'Minutos'].includes(activeGroup.minAnticipationType) ? 'rounded-r-xl border-l-0' : 'rounded-xl'}`}
                 >
              ...
              {['Día(s) y', 'Horas', 'Minutos'].includes(activeGroup.minAnticipationType) && (
                 <p className="text-[13px] text-slate-500 mt-2.5 font-medium leading-relaxed">Este grupo no recibirá citas para el próximo {activeGroup.minAnticipationValue || 0} {activeGroup.minAnticipationType.replace('Día(s) y', 'Día(s)').trim()}</p>
              )}
           </div>

           <div className="md:col-span-2">
              <label className="flex items-center text-[13px] font-semibold text-slate-800 mb-3">
                 Bloqueos de calendario puntuales (Vacaciones / Festivos) <Info className="w-3.5 h-3.5 text-slate-400 ml-1.5 cursor-help" />
              </label>
              <div className="flex flex-col sm:flex-row gap-3">
                 ...
                 <button 
                   ...
                   className="bg-black hover:bg-slate-900 text-white font-bold text-[13px] px-6 py-3 rounded-xl transition-colors disabled:opacity-50 cursor-pointer shadow-sm"
                 >
                   AGREGAR
                 </button>
              </div>

              <div className="mt-5 border border-slate-200/60 rounded-xl p-4 flex flex-wrap gap-2.5 max-w-3xl min-h-[70px] bg-[#f5f5f7]/30">
                 {activeGroup.blockedDates.map(date => (
                    <div key={date} className="bg-white border border-rose-100 text-rose-500 text-[13px] font-semibold px-3 py-1.5 rounded-lg flex items-center shadow-sm">
                 ...
                 {activeGroup.blockedDates.length === 0 && (
                   <span className="text-slate-400 text-[13px] font-medium flex items-center italic">Ninguna fecha específica bloqueada.</span>
                 )}
               </div>
           </div>
        </div>
        ...
        <div className="border border-slate-200/60 rounded-xl overflow-hidden mt-10 bg-white max-w-3xl shadow-sm">
          <div 
            onClick={() => setShowAdvanced(!showAdvanced)} 
            className="p-5 flex justify-between items-center cursor-pointer hover:bg-slate-50 transition-colors select-none"
          >
             <div className="flex items-center text-slate-700 font-semibold text-[14px]">
                <SettingsIcon className="w-4 h-4 mr-2 text-slate-600" /> Configuraciones avanzadas
             </div>
             <svg className={`w-5 h-5 text-slate-500 transform transition-transform ${showAdvanced ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M19 9l-7 7-7-7" /></svg>
          </div>
          
          <div className={`overflow-hidden transition-all duration-300 ease-in-out ${showAdvanced ? 'max-h-[1200px] opacity-100 border-t border-slate-200/60' : 'max-h-0 opacity-0'}`}>
             <div className="p-7 space-y-7">
               {/* Aprobación de nuevas citas */}
               <div>
                 <label className="flex items-center text-[13px] font-semibold text-slate-600 mb-3">
                   Aprobación de nuevas citas <Info className="w-3.5 h-3.5 text-slate-400 ml-1.5 cursor-help" />
                 </label>
                 <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                   <label className="flex items-center cursor-pointer group">
                     <span className={`w-4 h-4 rounded-full border-[5px] mr-2 transition-colors ${activeGroup.approvalType === 'Aprobación automática' ? 'border-black bg-white' : 'border-slate-300 bg-white group-hover:border-slate-400'}`}></span>
                     ...
                     <span className="text-[14px] text-slate-700 font-medium">Aprobación automática</span>
                   </label>
                   <label className="flex items-center cursor-pointer group">
                     <span className={`w-4 h-4 rounded-full border-[5px] mr-2 transition-colors ${activeGroup.approvalType === 'Aprobación manual' ? 'border-black bg-white' : 'border-slate-300 bg-white group-hover:border-slate-400'}`}></span>
                     ...
                     <span className="text-[14px] text-slate-700 font-medium">Aprobación manual</span>
                   </label>
                 </div>
               </div>
               ...
               <div>
                 <label className="flex items-center text-[13px] font-semibold text-slate-600 mb-3">
                   Permitir que los suscriptores cancelen sus citas <Info className="w-3.5 h-3.5 text-slate-400 ml-1.5 cursor-help" />
                 </label>
                 <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                   <label className="flex items-center cursor-pointer group">
                     <span className={`w-4 h-4 rounded-full border-[5px] mr-2 transition-colors ${activeGroup.allowCancel === 'Acepte cancelación' ? 'border-black bg-white' : 'border-slate-300 bg-white group-hover:border-slate-400'}`}></span>
                     ...
                     <span className="text-[14px] text-slate-700 font-medium">Acepte cancelación</span>
                   </label>
                   <label className="flex items-center cursor-pointer group">
                     <span className={`w-4 h-4 rounded-full border-[5px] mr-2 transition-colors ${activeGroup.allowCancel === 'No acepte cancelación' ? 'border-black bg-white' : 'border-slate-300 bg-white group-hover:border-slate-400'}`}></span>
                     ...
                     <span className="text-[14px] text-slate-700 font-medium">No acepte cancelación</span>
                   </label>
                 </div>
               </div>
               ...
               <div>
                 <label className="flex items-center text-[13px] font-semibold text-slate-600 mb-3">
                   Permitir a los suscriptores reprogramar sus horarios <Info className="w-3.5 h-3.5 text-slate-400 ml-1.5 cursor-help" />
                 </label>
                 <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                   <label className="flex items-center cursor-pointer group">
                     <span className={`w-4 h-4 rounded-full border-[5px] mr-2 transition-colors ${activeGroup.allowReschedule === 'Aceptar reprogramación' ? 'border-black bg-white' : 'border-slate-300 bg-white group-hover:border-slate-400'}`}></span>
                     ...
                     <span className="text-[14px] text-slate-700 font-medium">Aceptar reprogramación</span>
                   </label>
                   <label className="flex items-center cursor-pointer group">
                     <span className={`w-4 h-4 rounded-full border-[5px] mr-2 transition-colors ${activeGroup.allowReschedule === 'No se aceptan reprogramaciones' ? 'border-black bg-white' : 'border-slate-300 bg-white group-hover:border-slate-400'}`}></span>
                     ...
                     <span className="text-[14px] text-slate-700 font-medium">No se aceptan reprogramaciones</span>
                   </label>
                 </div>
               </div>
               ...
               <div>
                 <label className="flex items-center text-[13px] font-semibold text-slate-600 mb-3">
                   Visualización de las horas no disponibles <Info className="w-3.5 h-3.5 text-slate-400 ml-1.5 cursor-help" />
                 </label>
                 <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                   <label className="flex items-center cursor-pointer group">
                     <span className={`w-4 h-4 rounded-full border-[5px] mr-2 transition-colors ${activeGroup.unavailableDisplay === 'Mostrar como ocupado' ? 'border-black bg-white' : 'border-slate-300 bg-white group-hover:border-slate-400'}`}></span>
                     ...
                     <span className="text-[14px] text-slate-700 font-medium">Mostrar como ocupado</span>
                   </label>
                   <label className="flex items-center cursor-pointer group">
                     <span className={`w-4 h-4 rounded-full border-[5px] mr-2 transition-colors ${activeGroup.unavailableDisplay === 'Oculto' ? 'border-black bg-white' : 'border-slate-300 bg-white group-hover:border-slate-400'}`}></span>
                     ...
                     <span className="text-[14px] text-slate-700 font-medium">Oculto</span>
                   </label>
                 </div>
               </div>
               ...
               <div>
                 <label className="flex items-center text-[13px] font-semibold text-slate-600 mb-2">
                   Límite de programación por tiempo <Info className="w-3.5 h-3.5 text-slate-400 ml-1.5 cursor-help" />
                 </label>
                 <input 
                   type="number"
                   value={activeGroup.timeLimit}
                   onChange={(e) => updateGroup({ timeLimit: e.target.value })}
                   className="w-full max-w-sm bg-white border border-slate-200 focus:border-slate-400 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-4 focus:ring-black/10 text-slate-700 shadow-sm transition-all"
                 />
               </div>
               ...
               <div>
                 <label className="flex items-center text-[13px] font-semibold text-slate-600 mb-2">
                   Límite de programación por correo electrónico <Info className="w-3.5 h-3.5 text-slate-400 ml-1.5 cursor-help" />
                 </label>
                 <select 
                   value={activeGroup.emailLimit}
                   onChange={(e) => updateGroup({ emailLimit: e.target.value })}
                   className="w-full max-w-sm bg-white border border-slate-200 focus:border-slate-400 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-4 focus:ring-black/10 text-slate-700 shadow-sm transition-all cursor-pointer"
                 >
               ...
               <div>
                 <label className="flex items-center text-[13px] font-semibold text-slate-600 mb-2">
                   Antiguo <Info className="w-3.5 h-3.5 text-slate-400 ml-1.5 cursor-help" />
                 </label>
                 <select 
        ...
        <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl border border-slate-100 shadow-2xl max-w-md w-full p-6 relative">
            <button 
              ...
              className="absolute top-4 right-4 p-1 rounded-full bg-slate-50 hover:bg-slate-100 text-slate-400 hover:text-slate-600 transition-colors cursor-pointer"
            >
              <X className="w-4 h-4" />
            </button>
            <h3 className="text-base font-bold text-slate-800 mb-2">Añadir Administrador</h3>
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-2">Nombre completo</label>
                <input 
                  type="text" 
                  value={newAdminName} 
                  onChange={(e) => setNewAdminName(e.target.value)}
                  placeholder="Ej. Ana María"
                  className="w-full bg-[#f5f5f7] border border-transparent focus:bg-white focus:border-slate-300 rounded-xl px-4 py-3 text-sm text-slate-900 outline-none focus:ring-2 focus:ring-black/20 transition-all"
                  autoFocus
                />
              </div>
              <div className="flex gap-3 justify-end pt-2">
                <button 
                  onClick={() => { setIsAdminModalOpen(false); setNewAdminName(''); }} 
                  className="px-4 py-2 border border-slate-200 hover:bg-slate-50 text-slate-600 rounded-lg text-xs font-semibold cursor-pointer transition-all"
                >
                  Cancelar
                </button>
                <button 
                  ...
                  className="px-4 py-2 bg-black hover:bg-slate-900 disabled:opacity-50 text-white rounded-lg text-xs font-semibold cursor-pointer transition-all shadow-md shadow-slate-950/10"
                >
                  Añadir
                </button>
              </div>
