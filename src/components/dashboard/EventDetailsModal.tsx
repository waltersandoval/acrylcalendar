  ...
  return (
    ...
    <Sheet isOpen onClose={onClose} maxWidthClass="max-w-lg" zIndex={50}>
      <div className="srf-panel">
        <div className="flex justify-between items-center p-4 bg-[#f1f3f6] border-b hairline sticky top-0 z-10">
          <h2 className="text-[16px] sm:text-[17px] font-bold text-[#374151] truncate pr-4">{event.client} - {event.service}</h2>
          <button onClick={onClose} className="p-1 rounded-full bg-[#e2e8f0] hover:bg-[#cbd5e1] ink-3 cursor-pointer flex-shrink-0 transition-colors">
            <X className="w-5 h-5" />
          </button>
        </div>

        <div className="p-5 sm:p-6 space-y-6 srf-panel">
          
          <div className="space-y-4">
            <h3 className="text-[15px] font-bold ink-1">Programación</h3>
            <div className="flex items-center text-[13px] ink-2 gap-3">
              <span className={`w-2.5 h-2.5 rounded-full ${event.statusColor || 'bg-[#13CE9C]'} ml-0.5`}></span>
              {statusDisplay}
            </div>
            <div className="flex items-center text-[13px] ink-3 gap-3">
              <Calendar className="w-4 h-4 text-[#aec0d5]" />
              <span className="capitalize">{formattedDate}</span>
            </div>
            <div className="flex items-center text-[13px] ink-3 gap-3 pb-4 border-b hairline">
          ...
          <div className="space-y-4">
            <h3 className="text-[15px] font-bold ink-1">Pago</h3>
            <div className="flex items-center text-[13px] ink-2 gap-3 pb-4 border-b hairline">
          ...
          <div className="space-y-4">
            <h3 className="text-[15px] font-bold ink-1">Calendario</h3>
            <div className="flex items-center text-[13px] ink-3 gap-3">
              <Calendar className="w-4 h-4 text-[#aec0d5]" />
          ...
          <div className="space-y-4">
            <h3 className="text-[15px] font-bold ink-1">Suscriptores</h3>
            <div className="flex items-center text-[13px] ink-3 gap-3">
            ...
            <div className="flex items-start text-[13px] ink-3 gap-3 pb-4 border-b hairline">
              <div className="w-4 h-4 rounded bg-[#e2e8f0] text-[#94a3b8] flex justify-center items-center font-bold text-[10px] mt-0.5">
                {event.termsAccepted ? '✓' : ''}
              </div>
              <span className="leading-tight"><strong className="ink-2 font-semibold">¿Aceptas los términos y condiciones y la política de privacidad?</strong> {event.termsAccepted ? 'Aceptar' : 'Rechazado'}</span>
            </div>
          </div>

          <div className="space-y-3">
            <h3 className="text-[15px] font-bold ink-1">Notas internas</h3>
            <div className="flex items-center text-[13px] ink-3 gap-3">
              ...
              <input 
                 ...
                 className="bg-transparent border-none outline-none flex-1 ink-2 placeholder:ink-3"
              />
            </div>
          </div>
        ...
        <div className="border-t hairline bg-[#f8f9fa] p-4 sm:p-5 flex flex-col-reverse sm:flex-row sm:justify-between sm:items-center gap-3">
           <button onClick={() => setShowReschedule(true)} className="w-full sm:w-auto accent-bg hover:brightness-110 text-white font-bold py-3 px-6 rounded-lg flex justify-center items-center gap-2 cursor-pointer transition-colors text-[13px] tracking-wide">
      ...
      <div className="srf-panel p-6">
        ...
        <div className="flex flex-col-reverse sm:flex-row gap-3 sm:justify-end">
          <button
            ...
            className="px-4 py-2.5 ink-2 font-medium hover:srf-sunken rounded-lg cursor-pointer transition-colors text-sm"
          >
            Atrás
          </button>
          <button
