...
const AppointmentsView: React.FC<AppointmentsViewProps> = ({ calendarFilter, setCalendarFilter }) => {
      ...
      <div className="flex flex-col gap-4 pb-4">
        ...
        <div className="flex items-start justify-between gap-3 pt-1 pr-12">
          <div className="min-w-0">
            <h1 className="text-[32px] leading-[1.05] font-extrabold tracking-tight text-slate-900 font-display">Citas</h1>
          ...
          <button
            ...
            className="shrink-0 bg-black text-white h-10 px-4 rounded-2xl text-[14px] font-bold flex items-center gap-1.5 shadow-md active:scale-95 transition-transform"
          >
        ...
        <div className="flex items-center gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full bg-white border border-slate-200 rounded-2xl pl-10 pr-9 py-3 text-[15px] text-slate-700 outline-none focus:ring-2 focus:ring-black/15 shadow-sm"
            />
            {search && (
          ...
          <button
            ...
            className={`relative shrink-0 w-11 h-11 rounded-2xl border flex items-center justify-center transition-colors ${showFilters || activeFiltersCount > 0 ? 'bg-black border-black text-white' : 'bg-white border-slate-200 text-slate-500'}`}
            aria-label="Filtros"
          >
            ...
            return (
              ...
              >
                ...
                {s.id !== 'all' && counts[s.id] > 0 && (
                  <span className={`ml-1.5 text-[11px] ${active ? 'text-slate-300' : 'text-slate-400'}`}>
                    {counts[s.id]}
                  </span>
                )}
              </button>
        ...
        {showFilters && (
          <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm p-4 flex flex-col gap-3">
            <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Filtros avanzados</p>

            {/* Calendar filter */}
            <div className="relative">
              <CalendarIcon className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 pointer-events-none" />
              <select value={calendarFilter || ''} onChange={(e) => setCalendarFilter(e.target.value || null)} className="appearance-none w-full bg-slate-50 border border-slate-200 text-slate-700 text-[14px] font-medium rounded-xl pl-9 pr-8 py-3 outline-none focus:border-black">
            ...
            {admins.length > 0 && (
              <div className="relative">
                ...
                <select value={adminFilter} onChange={(e) => setAdminFilter(e.target.value)} className="appearance-none w-full bg-slate-50 border border-slate-200 text-slate-700 text-[14px] font-medium rounded-xl pl-9 pr-8 py-3 outline-none focus:border-black">
            ...
            {/* Date range */}
            <div className="flex items-center gap-2">
              <input type="date" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} className="flex-1 bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-[13px] text-slate-600 outline-none focus:border-black" />
              <span className="text-slate-400 text-[10px] font-bold uppercase">–</span>
              <input type="date" value={dateTo} onChange={(e) => setDateTo(e.target.value)} className="flex-1 bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-[13px] text-slate-600 outline-none focus:border-black" />
            </div>
  ...
  // ─── ESCRITORIO ────────────────────────────────────────────────────────────
  return (
    <div className="bg-white border-x border-b border-t-0 border-slate-200/60 shadow-sm flex flex-col overflow-hidden flex-1 mb-6 rounded-b-3xl mx-4 md:mx-6">
      <CalendarHeader
