    ...
    <div className="flex flex-col flex-1 h-full max-w-5xl mx-auto w-full pb-20">
      ...
      <div className="flex justify-between items-center mb-10 pt-4">
        <div className="flex items-center gap-4">
          {onBack && (
            <button onClick={onBack} className="w-10 h-10 flex items-center justify-center bg-white hover:bg-slate-50 text-slate-500 hover:text-slate-800 rounded-full border border-slate-200/60 transition-all duration-200 cursor-pointer shadow-sm">
          ...
          <h2 className="text-2xl font-bold tracking-tight text-slate-800 flex items-center">
            {calendarTitle}
            <div className="ml-3 w-8 h-8 flex items-center justify-center bg-slate-100 text-slate-800 rounded-lg text-sm border border-slate-200 shadow-sm">👤</div>
          </h2>
        </div>
        ...
        <label className="flex items-center cursor-pointer group">
          <div className="relative">
             ...
             <div className="block bg-slate-300 peer-checked:bg-black w-12 h-7 rounded-full transition-colors border border-slate-300/20 peer-checked:border-slate-900/20 shadow-inner"></div>
             <div className="dot absolute left-1 top-1 bg-white w-5 h-5 rounded-full transition-transform duration-300 transform peer-checked:translate-x-5 shadow-[0_2px_5px_rgba(0,0,0,0.2)]"></div>
          </div>
        </label>
      </div>
      ...
      {/* URL Box */}
      {isLinkGenerated ? (
        <div className="bg-white border border-slate-200/60 rounded-2xl p-4 flex flex-col md:flex-row items-center shadow-sm mb-10 gap-4 transition-all duration-500 animate-in fade-in slide-in-from-top-4">
          ...
          <div className="flex-1 w-full bg-[#f5f5f7] border border-transparent hover:border-slate-300 focus-within:border-slate-400 focus-within:ring-2 focus-within:ring-black/20 rounded-xl px-4 py-3 flex items-center transition-all">
             <input 
               ...
               readOnly 
               value={generatedLink}
               className="w-full bg-transparent text-slate-700 text-[13px] font-semibold outline-none"
             />
          </div>
          <div className="flex rounded-xl overflow-hidden flex-shrink-0 shadow-sm border border-slate-800/10">
            <button className="bg-black hover:bg-slate-900 px-4 py-3 text-white transition-colors cursor-pointer group tooltip-trigger relative">
              <Edit2 className="w-4 h-4 group-hover:scale-110 transition-transform" />
            </button>
            <button onClick={() => navigator.clipboard.writeText(generatedLink)} className="bg-black hover:bg-slate-900 px-4 py-3 text-white border-l border-slate-800/50 transition-colors cursor-pointer group tooltip-trigger relative">
            ...
            <button onClick={() => openExternalUrl(generatedLink)} className="bg-black hover:bg-slate-900 px-4 py-3 text-white border-l border-slate-800/50 transition-colors cursor-pointer group tooltip-trigger relative">
              <svg className="w-4 h-4 group-hover:scale-110 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
            </button>
          </div>
        </div>
      ) : (
        <div className="bg-slate-50/50 border border-slate-200/60 rounded-2xl p-6 flex items-center shadow-sm mb-10 gap-6 transition-colors hover:bg-slate-50">
           ...
           <div>
             <h3 className="font-semibold text-slate-800 text-[15px] tracking-tight">Enlace del calendario no generado</h3>
             <p className="text-[13px] text-slate-500 mt-1">Completa todas las configuraciones obligatorias para generar el enlace final permanente.</p>
           </div>
        </div>
      )}

      {/* Selector de Pasos */}
      <div className="flex flex-nowrap gap-1 bg-slate-100 p-1 rounded-xl border border-slate-200/50 mb-6 overflow-x-auto no-scrollbar scroll-smooth shrink-0">
        {[
          ...
          return (
            <button
              ...
              className={`flex-1 min-w-max md:min-w-0 px-4 py-2.5 rounded-lg text-xs font-bold transition-all duration-200 flex items-center justify-center gap-2 cursor-pointer whitespace-nowrap shrink-0 ${
                isActive
                  ? 'bg-white text-slate-900 shadow-sm border border-slate-200/30 font-extrabold scale-102'
